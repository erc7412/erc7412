...
library QueryType {
    ...
    function min() internal pure returns (uint8) {
        return ETH_CALL;
    }

    function max() internal pure returns (uint8) {
        return SOLANA_PDA;
    }
...
library QueryResponseLib {
    ...
    function verifyQueryResponse(address wormhole, bytes memory response, IWormhole.Signature[] memory signatures)
        internal
        view
    {
    ...
    function verifyQueryResponse(
        ...
        unchecked {
            ...
            while (true) {
                uint256 quorum = guardianSet.keys.length * 2 / 3 + 1;
                if (signatures.length >= quorum) {
                    (bool signaturesValid,) = wormhole_.verifySignatures(prefixedResponseHash, signatures, guardianSet);
                    if (signaturesValid) {
                        return;
                    }
                }
    ...
    function parseQueryResponse(bytes memory response) internal pure returns (QueryResponse memory ret) {
        unchecked {
            uint256 offset;
            ...
            uint256 respOff = offset + queryReqLen;
            uint256 startOfResponse = respOff;
            ...
            for (uint256 i; i < numPerChainQueries; ++i) {
    ...
    function parseEthCallQueryResponse(PerChainQueryResponse memory pcr)
        internal
        pure
        returns (EthCallQueryResponse memory ret)
    {
        unchecked {
            ...
            for (uint256 i; i < numBatchCallData; ++i) {
    ...
    function parseEthCallByTimestampQueryResponse(PerChainQueryResponse memory pcr)
        internal
        pure
        returns (EthCallByTimestampQueryResponse memory ret)
    {
        unchecked {
    ...
    function parseEthCallWithFinalityQueryResponse(PerChainQueryResponse memory pcr)
        internal
        pure
        returns (EthCallWithFinalityQueryResponse memory ret)
    {
        unchecked {
    ...
    function parseSolanaAccountQueryResponse(PerChainQueryResponse memory pcr)
        internal
        pure
        returns (SolanaAccountQueryResponse memory ret)
    {
        unchecked {
    ...
    function parseSolanaPdaQueryResponse(PerChainQueryResponse memory pcr)
        internal
        pure
        returns (SolanaPdaQueryResponse memory ret)
    {
        unchecked {
    ...
    function validateBlockTime(uint64 blockTimeInMicroSeconds, uint256 minBlockTimeInSeconds) internal pure {
    ...
    function validateChainId(uint16 chainId, uint16[] memory validChainIds) internal pure {
        unchecked {
            uint256 len = validChainIds.length;
    ...
    function _validateContractAddress(address contractAddress, address[] memory validContractAddresses) private pure {
        unchecked {
            uint256 len = validContractAddresses.length;
            for (uint256 i; i < len; ++i) {
    ...
    function _validateFunctionSignature(bytes4 functionSignature, bytes4[] memory validFunctionSignatures)
        private
        pure
    {
        unchecked {
            uint256 len = validFunctionSignatures.length;
            for (uint256 i; i < len; ++i) {
